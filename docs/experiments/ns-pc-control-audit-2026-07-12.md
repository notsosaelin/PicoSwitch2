...
| `0x10/0x01` firmware/version info | `02 00 11 02 0C 00 00 00 00 02 02 00` | `01 01 05 02 0C 00 00 00 FF FF FF FF` (confirmed **twice** independently in the previous pass — this repo's own capture AND `ndeadly`'s raw capture agree) | Real structural difference, not just per-unit noise — byte 3 (`0x02`, "Pro" type) matches in both, but the version-number bytes differ in shape, not just value | **Hypothesis: genuine firmware-version difference between physical units** (Nintendo pushes controller firmware updates; this field literally reports firmware version) — plausible, not verified either way | **Do not copy.** This repo's value is backed by two independent primary captures; NS-PC-Control's is a single unverifiable source. If ever revisited, the right question is "does the *console* accept multiple different values here" (probably yes, since firmware version legitimately varies), not "which one is correct." |
