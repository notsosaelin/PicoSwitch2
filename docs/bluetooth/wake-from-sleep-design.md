...
- `config.c` stores the validated controller address, up to two host addresses, and PID. Flash work
  is deferred five seconds so it cannot stall the remaining USB pairing handshake. Config v10
  retains the shared body/Sony-lightbar color, per-side Joy-Con 2 accents, and wake identity.
  A newly flashed UF2 deliberately clears this state and all bonds once; ordinary reboots retain
  it.
