...
void ns2_pairing_derive_ltk(const uint8_t a1_wire[16], const uint8_t device_key_b1[16],
    ...
    ns2_pairing_rev16(a1_wire, a1r);
    ns2_pairing_rev16(device_key_b1, b1r);
    for (int i = 0; i < 16; i++) ltk_out[i] = a1r[i] ^ b1r[i];
}

void ns2_pairing_challenge(const uint8_t ltk[16], const uint8_t a2_wire[16],
                           uint8_t b2_wire_out[16]) {
    uint8_t a2r[16];
    ns2_pairing_rev16(a2_wire, a2r);
    ns2_aes128_encrypt(ltk, a2r, b2_wire_out);
}
