...
Status: 🔵 **Partial — encoder complete and validated, not yet wired to any emission path.** It is
offline-validated only; both boards build clean and no behavior changed. Enabling it requires
sample buffering in the translation path, a default-off UART gate, and a gated hardware A/B.
