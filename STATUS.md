...
|---|---|---|
| **P1** | **In-band management ↔ controller disconnect/recovery failure** | 🟢 Strong HW evidence the decoupling fix (commit `68271a0`) works: a **5.4 h overnight soak** held a Classic controller + management client stable through **10 re-enumerations**, and when the controller finally idle-slept (clean `hci_disconnect reason 0x13`) **discovery resumed** (`scan.starts→2`, `suppress.mgmt_armed=0`) and management stayed connected — the adapter is healthy/scanning, **not** the power-cycle wedge. The original `disc=0` wedge was **not reproduced**, so sufficiency for that exact mechanism is inferred, not proven; the `pipe` diagnostic (built, unflashed) makes a recurrence decisive. **Open morning check:** does the slept controller reconnect on a button press. See [`docs/experiments/overnight-investigation-2026-08-13.md`](docs/experiments/overnight-investigation-2026-08-13.md) |
