...
|---|---|---|
| **P1** | **In-band management ↔ controller disconnect/recovery failure** | 🟡 Root cause (controller discovery/connect/wake gated on config/management state) fixed by fully decoupling the controller BT link (commit `68271a0`); live UART confirms concurrent scan+advertise and a stable Classic-controller + management-client session. HW soak in progress. Possibly shared with the CDC↔personality-switch failure. See [`docs/experiments/in-band-mgmt-coexistence-failure-2026-08-12.md`](docs/experiments/in-band-mgmt-coexistence-failure-2026-08-12.md) |
| **P2** | **Controlled re-enumeration after host-visible changes** | ⬜ Established requirement (owner): changes affecting descriptors/identity/exposed capabilities — controller **color**, mapping, personality-related config — only take effect after a USB re-enumeration. The management/config path needs a safe way to trigger re-enumeration when a change requires it (owner-observed: live color change did not apply until re-enumeration). Mechanism TBD safely; requirement tracked. Related: `personality <target>` already queues re-enumeration (G2) |
| **P3** | **Web Portal state refresh after Amiibo Sync** | ⬜ Frontend/state bug (owner-reported): after Sync Amiibo the data is fetched successfully but the portal shows stale owner/nickname/registration/last-write/write-count until a manual page refresh. Expected: sync → re-fetch amiibo status → auto-update the displayed metadata. `web/index.html` only |
