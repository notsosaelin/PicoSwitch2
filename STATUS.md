...
**The translation path is wired behind a default-off gate.** `ns2_ds5_motion40` buffers DualSense
samples and emits catch-up packets every ~20 ms, repeating the latest between USB polls exactly as
the `0x1E` path repeats its carrier. Toggle with `ds5motion pdu40 on|off`; `ds5motion pdu40 status`
reports emitted/starved/saturated counters. Enabling **replaces** the motion block — the `0x1E`
carrier is not sent alongside, because only `0x28`-only has a resolved elapsed relation.

Two defects the offline comparison caught before any flash:
