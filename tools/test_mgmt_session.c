 ...
 *
 * The dispatch gate calls the production mgmt_allow_write() implementation,
 * including its active-encryption requirement, rather than maintaining a
 * weaker test-local copy of that security boundary.
 *
 * gcc -std=c11 -Wall -Wextra -Werror -Isrc -Iinclude -Itools \
 *   tools/test_mgmt_session.c src/mgmt_access.c src/config_wireless_bridge.c \
...
static bool dispatch_allowed(const mgmt_state_t *s, const char *cmd) {
    return mgmt_allow_write(s, cmd);
}
...
static bool run_turn(const mgmt_state_t *s, const char *line, char *reply, size_t cap) {
    char framed[160];
...
static void test_bonded_user_command_succeeds(void) {
    ...
    mgmt_state_t s = {
        .enabled = true, .console_awake = true, .client_connected = true,
        .client_bonded = true, .client_encrypted = true,
    };
    char reply[128];
...
static void test_unbonded_client_cannot_command(void) {
    ...
    mgmt_state_t s = {
        .enabled = true, .console_awake = true, .client_connected = true,
        .client_bonded = false, .client_encrypted = true,
    };
    char reply[128];
...
static void test_bonded_plaintext_client_cannot_command(void) {
    config_wireless_bridge_init();
    mgmt_state_t s = {
        .enabled = true, .console_awake = true, .client_connected = true,
        .client_bonded = true, .client_encrypted = false,
    };
    char reply[128];
    assert(run_turn(&s, "save", reply, sizeof(reply)));
    assert(strstr(reply, "unavailable"));
    assert(run_turn(&s, "input active 1", reply, sizeof(reply)));
    assert(strstr(reply, "unavailable"));
}

static void test_encrypted_bond_without_live_connection_cannot_command(void) {
    config_wireless_bridge_init();
    mgmt_state_t s = {
        .enabled = true, .console_awake = true, .client_connected = false,
        .client_bonded = true, .client_encrypted = true,
    };
    char reply[128];
...
static void test_feature_disabled_rejects_everything(void) {
    config_wireless_bridge_init();
    mgmt_state_t s = {
        .enabled = false, .console_awake = true, .client_connected = true,
        .client_bonded = true, .client_encrypted = true,
    };
    char reply[128];
    assert(run_turn(&s, "get", reply, sizeof(reply)));
...
int main(void) {
    ...
    test_bonded_plaintext_client_cannot_command();
    test_encrypted_bond_without_live_connection_cannot_command();
