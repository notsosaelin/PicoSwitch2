...
class Adapter:
    ...
    def link_bound(self, state: dict | None = None) -> bool:
        ...
        state = self.state() if state is None else state
        return state.get("clink", {}).get("handle", HANDLE_NONE) != HANDLE_NONE
...
def await_link(timeout: float = 26.0) -> tuple[bool, float]:
    started = time.time()
...
def verdict(before: dict, after: dict, before_health: dict, after_health: dict,
    ...
    if delta("admission", "reject_window"):
        failures.append(
            f"companion admission rejects: {delta('admission', 'reject_window')}")
