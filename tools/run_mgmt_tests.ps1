...
Write-Host "== test_bluetooth_wipe_transport ==" -ForegroundColor Cyan
python tools/test_bluetooth_wipe_transport.py
if ($LASTEXITCODE -ne 0) {
    $fail++
    $failed += 'test_bluetooth_wipe_transport (run)'
} else {
    $pass++
}
...
Write-Host "== test_ns2_console_slot_wiring ==" -ForegroundColor Cyan
python tools/test_ns2_console_slot_wiring.py
if ($LASTEXITCODE -ne 0) {
    $fail++
    $failed += 'test_ns2_console_slot_wiring (run)'
} else {
    $pass++
}
