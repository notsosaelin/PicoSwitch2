...
7. On **Controller**, choose `Odin Controller`, prepare the bridge, open the adapter's physical
...
8. Rotate once while connected and once while the bridge is ready. Expect the destination,
   selection, colors, and bridge state to remain, with no repeated mutation.
9. If anything differs, open diagnostics and **Share privacy-safe diagnostics**. Send the text file
