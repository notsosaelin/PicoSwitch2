...
| Microphone / game audio from Android | Not possible over this link | `BluetoothHidDevice` carries HID reports only; there is no audio channel in the profile. A phone-audio path would need an A2DP/HFP transport plus a PicoSwitch2 A2DP sink and the USB microphone-return work PLAN.md still tracks, competing for the radio/CPU budget the DualSense audio path already saturates on Pico 2 W. Documented as a separate future subsystem rather than a bridge extension. |
