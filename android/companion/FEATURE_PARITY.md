...
| Microphone / game audio from Android | Not possible over the HID link; a WiFi path is credible but unbuilt | `BluetoothHidDevice` carries HID reports only — no audio channel exists in the profile, so the bridge itself can never carry audio. Investigating NS-PC-Control (which does support S2 audio/mic) showed it uses UAC1 on the console side, which PicoSwitch2 already has — **our UAC1 microphone endpoint is already operational and currently transmits silence** — and carries the audio to its client over **UDP/WiFi**. So the missing piece is only the adapter↔phone transport. WiFi is not compiled in today (`pico_cyw43_arch_none`) and the blocking unknown is CYW43 WiFi/BT coexistence on a shared radio; it would be offered only when the handheld is the sole source, so it would never compete with the DualSense Opus path. Assessment and recommended order: `docs/bluetooth/android-audio-feasibility-2026-08-13.md`. |
