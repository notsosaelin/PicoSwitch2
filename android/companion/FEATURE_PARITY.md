...
| Owner/nickname/dates/write count | Implemented with optional local keys | The portal-tested amiitool-compatible decrypt/read path accepts a validated 160-byte user key file. HMAC-invalid images expose no decrypted fields; keys remain app-private and are never sent, logged, or exported. |
| Amiibo initialization | Intentionally deferred | The page is read-only for encrypted metadata in this slice. Client-side wipe/re-sign still needs a separately tested local replacement transaction; it is not a firmware command. |
| Catalog artwork/games | Implemented as optional cache-first enrichment | AmiiboAPI `?showgames` metadata is matched by the portal's uppercase head+tail figure ID, cached privately for seven days, bounded to 4 MiB and two short-timeout mirrors, and never gates local import or adapter operations. Friendly name/character/series/type/release, compatible games, title-ID game labels, and best-effort artwork appear in the responsive detail pane; offline fallback keeps raw local identity usable. |
| ZIP library exchange | Intentionally deferred | Android keeps the same private per-file library and dirty safeguards, but does not yet expose the portal's full-library ZIP import/export transaction. |
...
| Android controller HID bridge | Partial Thor hardware pass | Thor live input and public HID registration reach Ready. The missing Companion Device Manager manifest feature that crashed pairing is fixed and guarded. Registration now follows the asynchronous callback because this OEM can return immediate `false` and then succeed; the rebuilt APK reaches `Connecting` to the bonded Pico. Pico receipt and console input remain. No root/Shizuku is used by this app. |
